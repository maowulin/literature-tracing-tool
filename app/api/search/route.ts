...
function convertCrossrefToLiterature(crossrefWork: any, id: number, query: string): Literature {
...
function convertExaResultToLiterature(exaResult: ExaResult, id: number, query: string): Literature {
  ...
  return {
    ...
    highlightedAbstract: wrapMarksByHighlights(abstractText, exaResult.highlights) || wrapMarksByQuery(query, abstractText),
  }
}
      ...
      const crossrefPromises = sentences.map(async (sentence, index) => {
        try {
          ...
          const literatureFromDois = doiResults
            ...
            .map((work, i) => convertCrossrefToLiterature(work, 1000 + i, sentence))
          ...
          const literatureFromBiblio = bibliographicResults.map((work, i) => convertCrossrefToLiterature(work, 2000 + i, sentence))
      ...
      const api1Results: SentenceResult[] = await Promise.all(
        sentences.map(async (sentence, index) => {
          ...
          const literature = exaResultsForSentence
            ...
            .map(exaResult => convertExaResultToLiterature(exaResult, literatureId++, sentence))
...
function wrapMarksByHighlights(text?: string, highlights?: string[]): string | undefined {
  if (!text) return undefined
