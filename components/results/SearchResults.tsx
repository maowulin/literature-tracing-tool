...
export function SearchResults({ results, searchQuery }: SearchResultsProps) {
  ...
  const totalCount = results.results.reduce(
    (sum, result) => sum + result.literature.length,
    0
  );
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            检索结果
            <Badge variant="secondary">共找到 {totalCount} 篇文献</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {results.results.length > 0 ? (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                <p className="text-muted-foreground">暂无相关文献结果</p>
              </CardContent>
            </Card>
          )}
        </CardContent>
